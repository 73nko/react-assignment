...
export class BackendService {
  ...
  storedUsers: User[] = [{ id: 111, name: "Victor" }, { id: 112, name: "Alex" }];
  ...
  updateTicket(ticketId: number, payload: { description: string, assigneeId: number }) {
  ...
  updateStatus(ticketId: number, completed: boolean) {
    const foundTicket = this.findTicketById(+ticketId);
    if (foundTicket) {
      return of(foundTicket).pipe(
        delay(randomDelay()),
